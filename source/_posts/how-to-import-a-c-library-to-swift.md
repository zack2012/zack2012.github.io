...
原文件放在Sources目录下，Sources下的每个子文件夹都是一个Target。最开始SwiftPM不支持自定义文件目录布局，但在以后的实践中的发现，不支持自定义文件目录很不方便，尤其是在导入一些c库时，这些c库有着很复杂的文件结构，所以在后面的版本里又加上自定义文件目录的功能。
...
这里多了一种新的target：systemLibrary。path需要传入这个target的文件夹的相对路径，路径的起点就是package.swift所在的目录，所以CGLFW的相对路径是"Sources/CGLFW"。这里最关键的参数就是pkgConfig，对于glfw我们需要传入"glfw3"，哪这个值从哪来？最简单的方法是通过下面命令获得：
...
1、为什么需要而外的shim.h这个文件？不能直接引用glfw的头文件吗？ 
...
将MoltenVK整个文件夹拷入`/usr/local/Cellar`，为了让pkg-config找到我们的刚才编写的vulkan.vc，需要把vulkan.pc拷入 `/usr/local/lib/pkgconfig`文件夹下，我们也可以用另一种方式，就是使用ln命令创建软链接：

```bash
ln -s path/to/vulkan.vc /usr/local/lib/pkgconfig/vulkan.pc
```

-s表示创建软链接，第一个路径是源文件路径，第二个路径目标路径。

同理，我们需要把头文件夹vulkan放到`/usr/local/include`下，把库文件libMoltenVK.dylib放到`/usr/local/lib`下，这里都采用软链接的方式：
...
完成以上步骤后我们就可以用SwiftPM的pkg-config来导入vulkan了。SwiftPM的编写与之前引入的glfw是一样的。  
